...
2. **Configure authentication**:
   - Ask: "How would you like to authenticate with Codex?"
     - **API Key** (recommended): Ask user to paste their ANTHROPIC_API_KEY
     - **Codex Subscription** (Pro/Max): Tell user to run `Codex setup-token` in another terminal, then paste CLAUDE_CODE_OAUTH_TOKEN
